...
- Admin UI is a real Vue/Vite single-page app under `web/admin`, built into `internal/app/admin_dist`, and embedded by the Go service. The old server-side inline HTML admin page is not used.
...
The Docker build includes a Node stage that runs the admin Vue build before compiling Go. For local frontend work, use:
...
The build output is embedded from `internal/app/admin_dist`; do not edit files in that directory by hand. Change Vue source files under `web/admin/src` and rebuild.
...
To update the existing hk-16-16 candidate stack from this working tree, run [deploy/hk-16-16-update-candidate.sh](deploy/hk-16-16-update-candidate.sh). It syncs the current Go/Vue admin source to `/root/go-shkeeper`, updates the server [deploy/shkeeperctl.sh](deploy/shkeeperctl.sh), calls `shkeeperctl.sh source-upgrade`, preserves the server `.env` and compose files, and verifies `/readyz`.
